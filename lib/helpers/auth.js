...
// This function is use for encrypt data what we send..
export const encryptResponse = async (originalData) => {
  var ciphertext = CryptoJS.AES.encrypt(JSON.stringify(originalData), appConfig.RESPONSE_AES_SECRET).toString()
  return ciphertext
}

// This function is use for decrypt data what we send..
export const decryptResponse = async (originalData) => {
  var bytes = CryptoJS.AES.decrypt(originalData, appConfig.RESPONSE_AES_SECRET)
